        ...
        // パラメータがあれば上書き、なければconfig.jsを参照
        // account を優先し、なければ従来の name も確認する
        const activeClinicName = urlParams.get('account') || urlParams.get('name') || (typeof CLINIC_NAME !== 'undefined' ? CLINIC_NAME : "");
        const activeLineId = urlParams.get('id') || (typeof CLINIC_LINE_ID !== 'undefined' ? CLINIC_LINE_ID : "");
        ...
        // 状態管理
        let userData = {
            treatment: "", // ortho or other
            experience: "", // first or past
            type: "", // new or re
        ...
        function goToStep(stepId) {
            ...
            if (stepId === 'step-info') {
                ...
                // 画面表示時に現在の入力内容をチェック（オートフィル対策）
                checkInputs();
            }

            // Progress bar
            const steps = ['step-treatment', 'step-experience', 'step-type', 'step-classification', 'step-notes', 'step-calendar', 'step-info', 'step-confirm'];
        ...
        // 施術選択
        function selectTreatment(val) {
            userData.treatment = val;
            if (val === 'other') {
                goToStep('step-reject-past');
            } else {
                goToStep('step-experience');
            }
        }
        ...
        function goBackFromNotes() {
            if (userData.classification === 'adult') {
                goToStep('step-classification');
            } else {
                goToStep('step-consent');
            }
        ...
        function checkInputs() {
            ...
            if (btn && nameEl && kanaEl && telEl) {
                // 電話番号からハイフンを削除
                let telVal = telEl.value.replace(/[ー－-]/g, '').trim();
                telEl.value = telVal;

                const isValid = (nameEl.value.trim() !== "" && kanaEl.value.trim() !== "" && telVal !== "");
        ...
        function finalSubmit() {
            const typeLabel = userData.type === 'new' ? '新規予約' : '再予約';
            const message = buildMessage(typeLabel);
            const encodedMsg = encodeURIComponent(message);
            proceedToSubmit(activeLineId, typeLabel, encodedMsg);
        }

        function buildMessage(typeLabel) {
...
第3希望：${userData.choices[2].date} ${userData.choices[2].time}
${paymentLine}
※このメッセージを送信し、当院からの返信をお待ちください`;
        }


        function proceedToSubmit(dynamicId, typeLabel, encodedMsg) {
        ...
        function updateSummary() {
            ...
            document.getElementById('summary-display').innerHTML = `
                ...
                <div><strong>予約種別：</strong> ${typeLabel}</div>
