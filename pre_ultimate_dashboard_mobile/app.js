...
// ── CONFIGURATION ──
const CONFIG = {
    GAS_URL: "https://script.google.com/macros/s/AKfycbyVWruPCv4t-TBPFCdZz3CDSRen8ncJ1_ez-CR73CZiwK2DyCy4plcywDEqtV8GyfJmYw/exec", // スプレッドシート連携用URL
    ACCOUNTS: [
        "(ミラ総)", "(ミラc)", "(ミラmo)", "(ミラi)", "(ミラ介)", "(ミラgl)", "(ミラf)", "(ミラ医)",
        "(ミラid)", "(ミラtik)", "(梅h)", "(梅c)", "(梅mo)", "(梅i)", "(梅g)", "(梅介)", "(梅f)"
    ],
...
function renderAccountList() {
    ...
    const selectMobile = document.getElementById('account-select-mobile');
    const optionsHtml = `
...
function showView(viewId) {
    ...
    if (viewId === 'requests') {
        ...
        document.getElementById('page-title').textContent = '不定休カレンダー設定';
...
function renderTable() {
    ...
    if (state.filterAccount !== 'all') {
        list = list.filter(a => a.lineAccount === state.filterAccount);
    }
...
function filterByAccount(acc, el) {
    state.filterAccount = acc;
    if (el) {
        document.querySelectorAll('.account-list .nav-item').forEach(item => item.classList.remove('active'));
        el.classList.add('active');
    }
    
    // PC用・モバイル用のセレクトボックスの値を同期
    const select = document.getElementById('account-select');
    if (select && select.value !== acc) {
        select.value = acc;
    }
    const selectMobile = document.getElementById('account-select-mobile');
    if (selectMobile && selectMobile.value !== acc) {
        selectMobile.value = acc;
    }
    renderTable();
}
